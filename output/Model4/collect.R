...
result = c() 
for (n in n.sim) {
  filename = here("Model4", "data", paste0("Model4_n", n, ".txt"))
...
output=aggregate(cbind(result$phi1,result$len1, result$error), list(result$n,result$a), FUN=mean) 
...
filename = here("Figure", "Pro1_MSE4.eps")
postscript(filename)
par(mar=c(2,4,2,2))
p1=ggplot(data = data, mapping = aes(x = n, y = (MSE), group = a, color=a, pch=a)) +
...
ps.options(horizontal = F)
ps.options(height=4, width=5)
filename = here("Figure", "Pro1_CP4.eps")
postscript(filename)
par(mar=c(2,4,2,2))
p1=ggplot(data = data, mapping = aes(x = n, y = CP, group = a, color=a, pch=a)) +
  geom_line(linewidth=0.3)+
  geom_point(size=2)+
  labs("test")+
  xlab("n")+
