...
library(latex2exp)
library(hrbrthemes)
here::i_am("Model5/collect.R")
n.sim = c(20000, 50000, 100000)
b0 = c(-0.2, 0.3, 0.1)
output = c()
result = c()
for (n in n.sim) {
  filename = here("Model5", "data", paste0("Model5_n", n, ".txt"))
...
result$phi4 = (b0[4] >= result$l4 & b0[4] <= result$u4) # useless variable, ignore it
result$error = (b0[1] - result$b1)^2 + (b0[2] - result$b2)^2 + (b0[3] - result$b3)^2
...
result$len4 = result$u4 - result$l4 # useless variable, ignore it
...
filename = here("Figure", "CP1_1.eps")
postscript(filename)
par(mar = c(2, 4, 2, 2))
p1 = ggplot(data = data,
            mapping = aes(
...
ps.options(horizontal = F)
ps.options(height = 5, width = 4)
filename = here("Figure", "CP1_2.eps")
postscript(filename)
par(mar = c(2, 4, 2, 2))
p2 = ggplot(data = data,
            mapping = aes(
...
ps.options(horizontal = F)
ps.options(height = 5, width = 4)
filename = here("Figure", "CP1_3.eps")
postscript(filename)
par(mar = c(2, 4, 2, 2))
p3 = ggplot(data = data,
            mapping = aes(
